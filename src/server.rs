...
impl Server {
    ...
    pub fn start_server(&self) -> Result<()> {
        ...
        info!(
            "Starting server on {}, mining address: {}",
        ...
        info!("Listening on {}  Server listen...", &self.node_address);
    ...
    pub fn send_tx(&self, addr: &str, tx: &Transaction) -> Result<()> {
        info!("send tx message to: {} txid: {:?}", addr, &tx.id);
        let data = Txmsg {
            addr_from: self.node_address.clone(),
    ...
    fn send_get_blocks(&self, addr: &str) -> Result<()> {
        info!("send getblocks message to: {}", addr);
    ...
    fn send_get_data(&self, addr: &str, kind: &str, id: &str) -> Result<()> {
        info!(
            "send getdata message to: {} kind: {} id: {}",
            addr, kind, id
        );
        let data = GetDatamsg {
            addr_from: self.node_address.clone(),
    ...
    fn handle_inv(&self, msg: Invmsg) -> Result<()> {
        info!("receive handle inv msg: {:#?}", msg);
        if msg.kind == "block" {
...
fn bytes_to_cmd(bytes: &[u8]) -> Result<Message> {
    ...
    if cmd == "addr".as_bytes() {
        ...
        Err(format_err!(
            "unknown command: {} in the server",
            String::from_utf8(cmd.clone())?
        ))
    }
}
